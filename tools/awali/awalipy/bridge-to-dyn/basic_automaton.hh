...
namespace awali { namespace py {
  ...
  public:
    ...
    operator dyn::automaton_t() const { return dyn::automaton_t(aut_); }
    ...
    dyn::any_t string_to_weight(std::string const& s) const
    {
      return aut_->get_context()->get_weight(s);
    }
    ...
    dyn::state_t null_state() const { return aut_->null_state(); }
    ...
    void verify_existence_of_transition(dyn::transition_t trans,
    ...
    bool has_state(int s) const { return aut_->has_state(s + 2); }
    ...
    int add_state0() { return aut_->add_state() - 2; }
    ...
    dyn::transition_t set_eps_transition3(int src, int dst,
    ...
    std::string add_eps_transition3(int src, int dst, const std::string& weight)
    {
      verify_existence_of_state(src, "Source");
      verify_existence_of_state(dst, "Destination");
      return weight_to_string(aut_->add_eps_transition(
          src + 2, dst + 2, aut_->get_context()->get_weight(weight)));
    }
    ...
    std::string lmul_weight(dyn::transition_t tr, const std::string& weight)
    {
      verify_existence_of_transition(tr);
      return weight_to_string(
          aut_->lmul_weight(tr, aut_->get_context()->get_weight(weight)));
    }
    ...
    std::string rmul_weight(dyn::transition_t tr, const std::string& weight)
    {
      verify_existence_of_transition(tr);
      return weight_to_string(
          aut_->rmul_weight(tr, aut_->get_context()->get_weight(weight)));
    }
    ...
    //     }
    //
    //     std::vector<dyn::transition_t> all_transitions() const { // including
    //     initial and final
    //       return aut_->all_transitions();
    //     }
    //
    //     std::vector<dyn::transition_t> all_out(int s) const { // including
    //     final
    //       verify_existence_of_state(s);
    //       return aut_->all_out(s+2);
    //     }

    //     std::vector<dyn::transition_t> all_in(int s) const { // including
    //     initial
    //       verify_existence_of_state(s);
    //       return aut_->all_in(s+2);
    //     }

    bool is_transducer() const { return aut_->is_transducer(); }
    ...
    void strip_history() { aut_->strip_history(); }


    bool has_explicit_name(int s) const
    {
      verify_existence_of_state(s);
      return aut_->has_explicit_name(s + 2);
    }
    ...
    void set_state_name(int s, std::string name) const
    {
      verify_existence_of_state(s);
      return aut_->set_state_name(s + 2, name);
    }
    ...
    weightset_t get_weightset() { return weightset_t(aut_->get_context()); }

    //     std::string dot(bool history=false, bool horizontal = true) {
    //       std::ostringstream os;
    //       dyn::dot(((dyn::automaton_t) aut_),os, history, horizontal) <<
    //       std::endl; return os.str();
    //     }
    //
    std::string svg(bool history = false, bool horizontal = true) const
    {
      std::ostringstream os;
      dyn::put(((dyn::automaton_t) aut_), os,
    ...
    void release() { aut_.reset(); }
  };

  basic_automaton_t make_basic_automaton2(std::string alphabet,
                                          std::string semiring)
  {
  ...
  {
    dyn::pdfdisplay((dyn::automaton_t) aut,
                    {dyn::LAYOUT = (horizontal ? HORIZONTAL : VERTICAL),
                     dyn::KEEP_HISTORY = history});
  }
  ...
  void verify_existence_of_state(basic_automaton_t& aut, int state,
                                 std::string comment = "")
  {
