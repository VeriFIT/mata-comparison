...
namespace awali { namespace py {
    ...
    struct dynamic_load_of_dyn
    {
      dynamic_load_of_dyn()
      {
        ...
        for (std::string dir : dirs) {
          std::string path
              = std::string(dir + "/" + "libawalidyn"
                            + std::string(CMAKE_SHARED_LIBRARY_SUFFIX));
        ...
        if (open == nullptr)
          throw std::runtime_error(
              "dlopen of Awali C++ dynamic library failed.");
      }
  ...
  basic_automaton_t concatenate_(basic_automaton_t aut1, basic_automaton_t aut2)
  {
    return basic_automaton_t(
        dyn::concatenate((dyn::automaton_t) aut1, (dyn::automaton_t) aut2));
  }
  ...
  simple_automaton_t
  make_automaton_from_label_and_weight_description(labelset_description ls,
  weightset_description ws) { return
  simple_automaton_t(make_automaton_from_context(ls,ws));
  }

  context_t make_context(context_description cd);
  */

  //   simple_automaton_t make_weighted_automaton_(std::string alphabet,
  //   std::string semiring) {
  //     try {
  ...
  //   }
  //
  //   simple_automaton_t make_weighted_automaton_with_eps_(std::string
  //   alphabet, std::string semiring) {
  //     try {
  //       return simple_automaton_t(make_automaton_with_eps(alphabet,
  //       semiring));
  //     } catch(const std::runtime_error& e) {
  //       error_no=WRONG_SEMIRING;
  //       return simple_automaton_t(make_automaton(""));
  ...
  simple_automaton_t make_NFA_with_eps2(std::string alphabet,
                                        std::string semiring)
  {
  ...
  simple_automaton_t characteristic_(simple_automaton_t aut,
                                     std::string const& semiring)
  {
    //     try {
    return simple_automaton_t(
        dyn::characteristic((dyn::automaton_t) aut, semiring));
    //     } catch(const std::runtime_error& e) {
    //       error_no=WRONG_SEMIRING;
    //       return simple_automaton_t(make_automaton(""));
    //     }
  ...
  {
    auto res = dyn::internal::derivation(
        (dyn::ratexp_t) exp,
        ((dyn::ratexp_t) exp)->get_context()->get_label(label), breaking);
  ...
  //   simple_automaton_t derived_term_(simple_ratexp_t exp, bool breaking =
  //   false, bool keep_history = true) {
  //     return simple_automaton_t(dyn::derived_term((dyn::ratexp_t) exp,
  //     breaking, keep_history));
  //   }
  // determinize
  ...
  */
  ...
  {
    return aut.weight_to_string(dyn::eval((dyn::automaton_t) aut, word));
  }

  std::map<std::string, std::string> enumerate_(simple_automaton_t aut,
                                                unsigned max)
  {
    ...
    for (auto p : res) {
      ...
      word << p.first;
      simple_res.emplace(word.str(), aut.weight_to_string(p.second));
    }
    return simple_res;
  }

  std::map<std::string, std::string> shortest_(simple_automaton_t aut,
                                               unsigned max)
  {
  ...
  simple_automaton_t make_divkbaseb_(unsigned k, unsigned b,
                                     std::string alphabet)
  {
    return simple_automaton_t(dyn::factory::divkbaseb(k, b, alphabet));
  }

  simple_automaton_t make_double_ring_(unsigned n,
  ...
  //       return
  //       simple_automaton_t(dyn::minimal_automaton((dyn::automaton_t)aut,
  //       BRZOZOWSKI));
  //    }

  simple_automaton_t minimal_automaton_(simple_automaton_t aut,
                                        std::string method)
  {
    ...
    if (method == "moore")
      ...
      throw std::domain_error(
          "Valid values for minimal_automaton are: \"hopcroft\", \"moore\" and "
          "\"brzozowski\"");

    return simple_automaton_t(
        dyn::minimal_automaton((dyn::automaton_t) aut, opts));
  }

  simple_automaton_t minimal_automaton_exp_(
      simple_ratexp_t exp, std::string const& exp_to_aut_method,
      std::string const& minim_method, std::string const& quotient_method)
  {
    awali::dyn::options_t opts
        = {dyn::MINIM_ALGO = minim_method, dyn::QUOTIENT_ALGO = quotient_method,
           dyn::EXP_TO_AUT_ALGO = exp_to_aut_method};

    return simple_automaton_t{
        dyn::minimal_automaton((dyn::ratexp_t) exp, opts)};
  }

  simple_automaton_t min_quotient_(simple_automaton_t aut,
                                   std::string method = "default")
  {
    return simple_automaton_t(dyn::min_quotient((dyn::automaton_t) aut,
                                                {dyn::QUOTIENT_ALGO = method}));
  }
  simple_automaton_t min_coquotient_(simple_automaton_t aut,
                                     std::string method = "default")
  {
    return simple_automaton_t(dyn::min_coquotient(
        (dyn::automaton_t) aut, {dyn::QUOTIENT_ALGO = method}));
  }

  simple_automaton_t quotient_(simple_automaton_t aut,
  ...
  simple_automaton_t coquotient_(simple_automaton_t aut,
                                 std::vector<std::vector<int>>& equiv)
  {
    auto a = (dyn::automaton_t) aut;
    std::vector<std::vector<dyn::state_t>> eqs;
  ...
  // output
  //   std::string dot_(simple_automaton_t aut, bool history=false, bool dot2tex
  //   = false) {
  ...
  {
    return simple_automaton_t(
        dyn::product((dyn::automaton_t) aut1, (dyn::automaton_t) aut2));
  }
  ...
  {
    return simple_automaton_t(
        dyn::shuffle((dyn::automaton_t) aut1, (dyn::automaton_t) aut2));
  }

  simple_automaton_t infiltration_(simple_automaton_t aut1,
                                   simple_automaton_t aut2)
  {
    return simple_automaton_t(
        dyn::infiltration((dyn::automaton_t) aut1, (dyn::automaton_t) aut2));
  }

  simple_automaton_t sum_(simple_automaton_t aut1, simple_automaton_t aut2)
  {
    return simple_automaton_t(
        dyn::sum((dyn::automaton_t) aut1, (dyn::automaton_t) aut2));
  }

  bool are_isomorphic_(basic_automaton_t aut1, basic_automaton_t aut2)
  {
    return dyn::are_isomorphic((dyn::automaton_t) aut1,
                               (dyn::automaton_t) aut2);
  }

  // proper
  void proper_here_(simple_automaton_t aut, bool backward = true,
                    bool prune = true)
  {
    dyn::proper((dyn::automaton_t) aut,
                {dyn::IN_PLACE = true,
                 dyn::DIRECTION = (backward ? BACKWARD : FORWARD),
                 dyn::PRUNE = prune});
  }

  simple_automaton_t proper_(simple_automaton_t aut, bool backward = true,
                             bool prune = true)
  {
    return simple_automaton_t(
        dyn::proper((dyn::automaton_t) aut,
  ...
  {
    return simple_automaton_t(
        dyn::allow_eps_transition((dyn::automaton_t) aut));
  }
  ...
  // ratexp
  /*
  ratexp_t make_ratexp_with_context(const std::string& exp, context_description
  cd); ratexp_t make_ratexp_with_context(const std::string& exp,
  labelset_description ls, weightset_description ws);
  */
  //   simple_ratexp_t make_weighted_ratexp_with_alphabet_(const std::string&
  //   exp, const std::string& alph, const std::string& sr) {
  //     return simple_ratexp_t(make_ratexp_with_alphabet(exp, alph, sr));
  //   }
  //
  //   simple_ratexp_t make_regexp_with_alphabet_(const std::string& exp, const
  //   std::string& alph) {
  //     return simple_ratexp_t(make_ratexp_with_alphabet(exp, alph));
  //   }
  //
  //   simple_ratexp_t make_weighted_ratexp_(const std::string& exp, const
  //   std::string& sr) {
  ...
  simple_ratexp_t aut_to_exp1_(basic_automaton_t aut)
  {
    return simple_ratexp_t(dyn::aut_to_exp((dyn::automaton_t) aut));
  }


  simple_ratexp_t aut_to_exp2_(basic_automaton_t aut, const std::string& order)
  {
    return simple_ratexp_t(dyn::aut_to_exp((dyn::automaton_t) aut,
                                           {dyn::STATE_ELIM_ORDER = order}));
  }
  ...
  //   }
  //
  //   void set_tdc_transition_(simple_automaton_t tdc, state_t src, state_t
  //   dst, const std::vector<std::string>& labels) {
  //     set_tdc_transition((dyn::automaton_t)tdc, src, dst, labels);
  //   }
  //
  //   void add_tdc_transition_(simple_automaton_t tdc, state_t src, state_t
  //   dst, const std::vector<std::string>& labels) {
  //     add_tdc_transition((dyn::automaton_t)tdc, src, dst, labels);
  //   }
  //
  //   void del_tdc_transition_(simple_automaton_t tdc, state_t src, state_t
  //   dst, const std::vector<std::string>& labels) {
  //     del_tdc_transition((dyn::automaton_t)tdc, src, dst, labels);
  //   }
  //
  //   void set_tdc_wtransition_(simple_automaton_t tdc, state_t src, state_t
  //   dst, const std::vector<std::string>& labels, const std::string& w) {
  //     set_tdc_transition((dyn::automaton_t)tdc, src, dst, labels, w);
  //   }
  //
  //   void add_tdc_wtransition_(simple_automaton_t tdc, state_t src, state_t
  //   dst, const std::vector<std::string>& labels, weight_t w) {
  //     add_tdc_transition((dyn::automaton_t)tdc, src, dst, labels, w);
  //   }

  //   std::vector<std::string> get_tdc_label_(simple_automaton_t tdc,
  //   transition_t tr) {
  ...
  simple_transducer_t compose_(simple_transducer_t tdc1,
                               simple_transducer_t tdc2)
  {
  ...
  basic_automaton_t load_automaton_(const std::string& s,
                                    const std::string& fmt)
  {
    return basic_automaton_t(dyn::load(s, {dyn::IO_FORMAT = fmt}));
  }

  void save_automaton2_(basic_automaton_t aut, const std::string& s)
  {
    dyn::save(aut, s);
  }

  void save_automaton3_(basic_automaton_t aut, const std::string& s,
                        const std::string& fmt)
  {
    dyn::save(aut, s, {dyn::IO_FORMAT = fmt});
  }
  void save_automaton4_(basic_automaton_t aut, const std::string& s,
                        const std::string& fmt, bool keep_history)
  {
    dyn::save(aut, s, {dyn::IO_FORMAT = fmt, dyn::KEEP_HISTORY = keep_history});
  }
  ...
  simple_automaton_t concatenate_(simple_automaton_t aut,
                                  simple_automaton_t aut2)
  {
    return simple_automaton_t(
  ...
  simple_automaton_t sum_of_standad_(simple_automaton_t aut,
                                     simple_automaton_t aut2)
  {
    return simple_automaton_t(
        dyn::sum_of_standard((dyn::automaton_t) aut, (dyn::automaton_t) aut2));
  }
  ...
  simple_automaton_t explore_by_length_(simple_automaton_t a, int i)
  {
    return simple_automaton_t(dyn::explore_by_length(a, i));
  }

  simple_automaton_t explore_with_bound(simple_automaton_t a,
                                        std::string const& s)
  {
    return simple_automaton_t(
        dyn::explore_with_bound(a, a.string_to_weight(s)));
  }
  ...
  basic_automaton_t promote_automaton_(basic_automaton_t aut,
                                       std::string weightset)
  {
    return dyn::promote_automaton(aut, weightset);
  }

  simple_ratexp_t promote_ratexp_(simple_ratexp_t exp, std::string weightset)
  {
    return dyn::promote_ratexp(exp, weightset);
  }
