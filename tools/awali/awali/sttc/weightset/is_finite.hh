...
namespace awali {
  namespace sttc {
    namespace internal {
      ...
      constexpr auto is_finite(const WS& ws, priority::TWO<P>) -> typename WS::finite_t {
        return WS::finite;
      }
                                                                   
      template<typename WS, typename P>
      constexpr bool is_finite(const WS& ws, priority::ONE<P>) {
        return false;
      }

      template<typename WS, typename P>
      constexpr auto is_locally_finite(const WS& ws, priority::TWO<P>) -> typename WS::locally_finite_t {
