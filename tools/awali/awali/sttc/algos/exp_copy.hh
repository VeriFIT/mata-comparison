...
#ifndef AWALI_ALGOS_EXP_COPY_HH
# define AWALI_ALGOS_EXP_COPY_HH
...
namespace awali { namespace sttc {
  ...
  namespace rat
  {
    ...
    template <typename RatExpSet,
              typename Weightset = weightset_t_of<RatExpSet>>
	...
	res_ = out_ratexpset_.plus(res_);
      ...
      {
        e.sub()->accept(*this);
	res_ = out_ratexpset_.lmul(out_weightset_.conv(in_weightset_, e.weight()), res_);
      }

      AWALI_RAT_VISIT(rweight, e)
      {
        e.sub()->accept(*this);
	res_ = out_ratexpset_.rmul(res_, out_weightset_.conv(in_weightset_, e.weight()));
     }
    ...
    private:
      const out_weightset_t& out_weightset_;
      const in_ratexpset_t& in_ratexpset_;
      out_context_t out_context_{*in_ratexpset_.labelset(), out_weightset_};
      out_ratexpset_t out_ratexpset_{out_context_, in_ratexpset_.identities()};
      in_weightset_t in_weightset_ = *in_ratexpset_.weightset();
      typename out_ratexpset_t::ratexp_t res_;
    };

  } // rat::

   /** @brief copy a rational expression
    *
    * @tparam OutWeightSet type of the weightset of the copy
    ...
    * @param out_weightset the weightset of the copy
    * @return a rational expression with the given weightset
    *
    * The function produces a fresh rational expression.
    * The \p out_weightset must be compliant with the weightset of \p ratexpset.
    * The ratexpset of the result has the same letterset and the same identities as \p ratexpset;
    */
    template <typename OutWeightSet,
	      typename RatExpSet
            >
  auto
    ratexp_copy(const typename RatExpSet::ratexp_t& exp,
  ...
  {
    rat::expcopy_visitor<RatExpSet, OutWeightSet> copyer{ratexpset,out_weightset};
    return copyer(exp);
  }
  ...
  {
    rat::expcopy_visitor<RatExpSet> copyer{ratexpset,*ratexpset.weightset()};
    return copyer(exp);
  }
   ...
   * @param out_weightset the weightset of the result
   * @return a ratexpset with the same letterset and the same identities as the original \p ratexpset, with a weightset equal to \p out_weightset.
   */									        template <typename OutWeightSet,
            typename RatExpSet>
  auto
  promote_ratexpset(const RatExpSet& ratexpset,
		    const OutWeightSet& out_weightset = OutWeightSet())
