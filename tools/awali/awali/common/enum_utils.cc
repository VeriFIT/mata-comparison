...
namespace internal {
  ...
  std::unordered_map<std::string,
                     std::pair<std::unordered_map<std::string, enum_join_t>,
                               std::unordered_map<enum_join_t, std::string>>>
  init_enum_map()
  {
    std::unordered_map<std::string,
                       std::pair<std::unordered_map<std::string, enum_join_t>,
...
#define REGISTER_ENUM_VALUE(type, name)                                 \
  ...
  map[#type].second[name] = #name
    ...
    REGISTER_ENUM_VALUE(exp_to_aut_algo_t, NET);
...
#undef REGISTER_ENUM_VALUE
  ...
  std::unordered_map<
      std::string,
      std::pair<std::unordered_map<std::string, enum_join_t>,
                std::unordered_map<enum_join_t, std::string>>> const enum_map
      = init_enum_map();


  //    std::vector<std::pair<std::string,
  //        std::pair< std::unordered_map<std::string,enum_join_t>,
  //                   std::unordered_map<enum_join_t, std::string> > > const* >
  //        init_enum_vector (
  //          std::unordered_map<
  //              std::string,
  //              std::pair< std::unordered_map<std::string,enum_join_t>,
  ...
  std::vector<enum_join_t> enum_values(std::string const& enum_name)
  {
    ...
    std::vector<enum_join_t> result;
    for (auto pair : it1->second.second)
                             ...
                             const std::string& value)
  {
    ...
    if (it2 == it1->second.first.end()) {
      ...
      for (auto pair : it1->second.first) {
        ss << sep << '"' << pair.first << '"';
        std::string other_name = it1->second.second.find(pair.second)->second;
        if (other_name != pair.first)
          ss << " (aka \"" << other_name << "\")";
        sep = ", ";
      }
  ...
  std::string string_of_enum(const std::string& enum_name, enum_join_t value)
  {
    auto it1 = enum_map.find(enum_name);
    if (it1 == enum_map.end())
      throw std::domain_error("No enum " + enum_name + " is registered.");
...
{
  return (exp_to_aut_algo_t) internal::enum_of_string("exp_to_aut_algo_t",
                                                      value);
}
...
{
  return (state_elim_order_t) internal::enum_of_string("state_elim_order_t",
                                                       value);
}
