...
protected:
  ...
  virtual void before_child(object_t const* node, std::string const& key,
                            node_t const* child) {}

  virtual void after_child(object_t const* node, std::string const& key,
                           node_t const* child) {}
  

  virtual void before_child(array_t const* node, unsigned index,
                            node_t const* child) {}

  virtual void after_child(array_t const* node, unsigned index,
                           node_t const* child) {}

  /** Function called just before the specialized version with the same name */
  virtual void before_child(node_t const* node, uint_or_string_t const& uos,
                           node_t const* child) {}

  /** Function called just after the specialized version with the same name */
  virtual void after_child(node_t const* node, uint_or_string_t const& uos,
                           node_t const* child) {}
