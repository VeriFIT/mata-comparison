...
namespace awali { namespace dyn { namespace internal {
  ...
  struct acceptor_t
  {
    virtual bool is_valid(T const&) { return true; }
    virtual std::string acceptable_values()
    {
      return ("any value of type "
  ...
  {
    virtual bool is_valid(bool const&) { return true; }
    std::string acceptable_values() { return ("boolean"); }
    virtual ~acceptor_t() = default;
  };
  ...
  {
    virtual bool is_valid(std::string const&) { return true; }
    virtual std::string acceptable_values() { return ("any string"); }
    virtual ~acceptor_t() = default;
  };
  ...
  using stov_t = std::function<option_value_pair_t(std::string const&)>;
  /* stov= string to value*/
  ...
  private:
    ...
    internal::option_value_pair_t make(const X&, priority::ONE<P>)
    {
      throw std::runtime_error("Wrong value type for option " + option_name
                               + ". Expected: " + acceptor.acceptable_values()
                               + ".");
    }

    template <typename X = T, typename P>
    internal::option_value_pair_t make(const char* value, priority::TWO<P>)
    {
    ...
    auto make(std::string const& value, priority::THREE<P>) ->
        typename std::enable_if<std::is_same<bool, X>::value,
                                internal::option_value_pair_t>::type
    {
      if (value == "default")
      ...
      if (value == "false")
        return {id, false};
      throw std::runtime_error("Wrong value for option " + option_name
                               + ". Expected a bool, got an std::string, thus expected one of the following values: \"default\", \"true\", \"false\".");
    }

    template <typename X = T, typename P>
    auto make(std::string const& value, priority::THREE<P>)
        -> decltype(awali::internal::make_enum<X>(value),
  ...
  public:
    option_t(std::string name, T def, acceptor_t<T> acc = acceptor_t<T>())
        : id(internal::next_id++)
        , option_name(std::move(name))
        , acceptor(acc)
        , default_value(def)
    {
      register_stov(option_name,
                    [this](std::string const& val) { return (*this) = val; });
    }

    bool acceptable(T value) { return acceptor(value); }
    ...
    operator internal::option_value_pair_t()
    {
      ...
      throw std::runtime_error(
          "Using option without argument is only possible if they expect "
          "boolean values.");
    }
     ...
     *    boolean inplace2 = opt[IN_PLACE.with_default_value(true)] // `true`
     * (local default)
     *
     *    IN_PLACE.set_default_value(false); // end user sets a default value
     *    boolean inplace3 = opt[IN_PLACE.with_default_value(true)] // 'false`
     * (user defined default value) \endcode
     */
    ...
    value_t get_default_value() { return default_value; }
