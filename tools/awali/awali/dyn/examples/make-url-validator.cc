...
int main()
{
  ...
  // Adds a state named "protocol"
  state_t protocol = aut->add_state("protocol");
  // Adds a state name "domain"
  state_t domain = aut->add_state("domain");
  // etc.
  state_t first_level = aut->add_state("first level");
  ...
  // Adds the states and transitions required such that every word that
  // match expression  "https?://"  labels a path from state `protocol` to
  // state `domain`.
  ...
  add_path(aut, path, name, "(/(['.0-9A-Z_a-z-]{1,}/)*['.0-9A-Z_a-z-]*)?",
           false);
